<?php
/**
 * TCF Registration Confirmation Page - Embeddable Version
 ...
 * L'ID de l'inscription est récupéré depuis l'URL: /confirmation/{id}
 */

// Get registration ID from URL path (format: /confirmation/{id})
$registrationId = 0;
$requestUri = $_SERVER['REQUEST_URI'] ?? '';
if (preg_match('/\/confirmation\/(\d+)/', $requestUri, $matches)) {
    $registrationId = intval($matches[1]);
}
...
if (!$registration) {
    ...
    // Get transactionId from Square callback URL
    ...
    if (!$registration['payment_confirmed']) {
        $updateStmt = $bdd->prepare("UPDATE tcf_registrations SET payment_confirmed = 1, payment_confirmed_at = NOW(), transaction_id = :transactionId WHERE id = :id");
        $updateStmt->execute([':id' => $registrationId, ':transactionId' => $transactionId]);
        $registration['payment_confirmed'] = 1;
        $registration['transaction_id'] = $transactionId;
    } elseif (empty($registration['transaction_id']) && !empty($transactionId)) {
        // Update transaction_id if it wasn't set before
        $updateStmt = $bdd->prepare("UPDATE tcf_registrations SET transaction_id = :transactionId WHERE id = :id");
        $updateStmt->execute([':id' => $registrationId, ':transactionId' => $transactionId]);
        $registration['transaction_id'] = $transactionId;
    }
}
